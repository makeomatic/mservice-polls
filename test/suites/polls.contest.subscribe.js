...
const http = request.defaults({
  uri: 'http://localhost:3000/api/polls/contest/subscribe',
  simple: false,
...
describe('polls.contest.subscribe', function suite() {
  ...
  it('should be able to return error if invalid params', () => {
    ...
    return http({ body: payload, headers: authHeader(this.userToken) })
      .then(({ body }) => {
        ...
        assert.equal(body.message, 'polls.contest.subscribe.request validation failed: data.id'
          + ' should be integer');
      });
  });
  ...
  it('should be able to return error if contest has questions', () => {
    ...
    return http({ body: payload, headers: authHeader(this.userToken) })
      .then(({ body }) => {
        assert.equal(body.statusCode, 403);
        assert.equal(body.message, 'An attempt was made to perform an operation that is not'
          + ' permitted: Contest has questions and can\'t be subscribed');
      });
  });
  ...
  it('should be able to return error if already subscribed', () => {
    const payload = {
      id: this.contestNoQuestions.get('id'),
    };
