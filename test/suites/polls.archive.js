...
const http = request.defaults({
  uri: 'http://localhost:3000/api/polls/archive',
  simple: false,
...
describe('polls.archive', function suite() {
  ...
  before('create poll with state ended', () => {
    const params = {
      title: 'What is your favorite cat?',
      ownerId: 'owner@poll.com',
      minUserAnswersCount: 1,
  ...
  it('should be able to return error if has not access', () => {
    ...
    const payload = { id: this.endedPoll.get('id') };
  ...
  it('should be able to return error if invalid params', () => {
    ...
    const payload = { id: this.endedPoll.get('id'), ownerId: 'owner@id.com' };

    return http({ headers, body: payload })
      .then(({ body }) => {
        assert.equal(body.statusCode, 400);
        assert.equal(body.message, 'polls.archive.request validation failed: data should NOT'
          + ' have additional properties');
      });
  });

  it('should be able to stop poll with status ended', () => {
    const payload = { id: this.endedPoll.get('id') };
  ...
  it('should not be able to stop poll with status created', () => {
    ...
    return http({ body: payload, headers: authHeader(this.rootToken) })
      .then(({ body }) => {
        assert.equal(body.statusCode, 403);
        assert.equal(body.message, 'An attempt was made to perform an operation that is not'
          + ' permitted: Can\'t archive poll that have not ended');
      });
  });
});
