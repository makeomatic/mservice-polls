...
const http = request.defaults({
  uri: 'http://localhost:3000/api/polls/contest/stop',
  simple: false,
...
describe('polls.contest.stop', function suite() {
  ...
  before('create contest with state created', () => {
    ...
    return polls
      .service('contest')
      .create(params)
      .tap((contest) => { this.startedContest = contest; });
  });
  ...
  before('create contest with poll', () => {
    const contestParams = {
      prize: 'Toronto FC Jersey',
      ownerId: 'owner@poll.com',
      state: polls.service('contest').constructor.state.STARTED,
      hasQuestions: true,
    };
  ...
  it('should be able to return error if has not access', () => {
    ...
    const payload = { id: this.startedContest.get('id') };
  ...
  it('should be able to return error if invalid params', () => {
    ...
    return http({ headers, body: payload })
      .then(({ body }) => {
        ...
        assert.equal(body.message, 'polls.contest.stop.request validation failed: data should NOT'
          + ' have additional properties');
      });
  });

  it('should be able to stop contest with status started', () => {
    const payload = { id: this.startedContest.get('id') };
    ...
    return http({ body: payload, headers: authHeader(this.rootToken) })
      .then(({ body }) => {
        ...
        assert.equal(attributes.state, 2);
  ...
  it('should be able to stop contest with poll', () => {
